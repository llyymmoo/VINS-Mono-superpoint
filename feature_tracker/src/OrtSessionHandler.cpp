...
void OrtSessionHandler::OrtSessionHandlerIml::initModelInfo()
{
    for (int i = 0; i < m_numInputs; i++) {
        ...
        auto input_name = m_session.GetInputNameAllocated(i, m_ortAllocator);
		inputNodeNameAllocatedStrings.push_back(std::move(input_name));
		m_inputNodeNames.push_back(inputNodeNameAllocatedStrings.back().get());
        // m_ortAllocator.Free(input_name.get());
    }
    ...
    for (int i = 0; i < m_numOutputs; ++i) {
        ...
        auto output_name = m_session.GetOutputNameAllocated(i, m_ortAllocator);
		outputNodeNameAllocatedStrings.push_back(std::move(output_name));
		m_outputNodeNames.push_back(outputNodeNameAllocatedStrings.back().get());
        // m_ortAllocator.Free(output_name.get());
    }
