...
KeyPointAndDesc SuperPoint::processOneFrame(const cv::Mat& inputImg, int maxnums, float* dst, int borderRemove,
                                            float confidenceThresh, bool alignCorners, int distThresh)
{
    ...
    preprocess(dst, scaledImg.data, Ort::SuperPoint::IMG_W, Ort::SuperPoint::IMG_H, Ort::SuperPoint::IMG_CHANNEL);
    ...
    if (maxnums <= 0)
        maxnums = keepIndices.size();
    else
        maxnums = (keepIndices.size() > maxnums) ? maxnums : keepIndices.size();
    keepKeyPoints.reserve(maxnums);
    std::transform(keepIndices.begin(), keepIndices.begin() + maxnums, std::back_inserter(keepKeyPoints),
                   [&keyPoints](int idx) { return keyPoints[idx]; });
    keyPoints = std::move(keepKeyPoints);

    cv::Mat descriptors = getDescriptors(coarseDescriptorMat, keyPoints, Ort::SuperPoint::IMG_H,
